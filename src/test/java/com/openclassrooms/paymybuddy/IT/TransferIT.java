...
public class TransferIT {
    ...
    public void getCurrentUserTransfersTest_whenCurrentUserIsDada_thenReturnListTransferOfDada() throws Exception {
        //GIVEN
        List<DisplayingTransfer> transfers = new ArrayList<>();

        DisplayingTransfer displayingTransfer1 = new DisplayingTransfer(
                "transfer paymybuddy", "credit", 50.0, 150.0);
        DisplayingTransfer displayingTransfer2 = new DisplayingTransfer(
                "transfer BNP", "debit", 30.0, 120.0);
        transfers.add(displayingTransfer1);
        transfers.add(displayingTransfer2);
        ...
        mockMvcTransfer.perform(MockMvcRequestBuilders.get("/transfer").with(SecurityMockMvcRequestPostProcessors.csrf())
                        ...
                        .param("transfers", String.valueOf(transfers)))
    ...
    public void addTransferCurrentUserTest_whenTransferTypeIsCredit_thenReturnBalanceCreditedWithAmount() throws Exception {
        ...
        User currentUser = User.builder()
                .email("dada@email.fr").balance(100.0).firstName("Damien").lastName("Sanchez")
                .build();
        Transfer transfer = Transfer.builder()
                .user(currentUser).userEmail(SecurityUtilities.userEmail).amount(20.0).type("credit").postTradeBalance(120.0)
                .build();
        //WHEN
        //THEN
